...
//  DataBinding
//
//  Created by Tuấn Phạm on 31/03/2022.
//
...
class NewImageController: UIViewController {
    ...
    @objc func dragView(gesture: UIPanGestureRecognizer) {
        ...
        switch gesture.state {
            ...
            if(originX_New >= -4 && originY_New >= -4)
            
            {
                target.frame.origin.x = origin.x + translation.x
//                target.frame.origin = CGPoint(x: origin.x + translation.x, y: origin.y + translation.y)
            }
            if(originY_New >= -4 && originY_New <= max_OriginY )
            
            {
                target.frame.origin.y = origin.y + translation.y
//                target.frame.origin = CGPoint(x: origin.x + translation.x, y: origin.y + translation.y)
            }
